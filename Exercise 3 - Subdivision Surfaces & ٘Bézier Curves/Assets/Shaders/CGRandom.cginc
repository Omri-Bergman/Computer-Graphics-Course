...
{
    // float2 b_l = random2(float2(floor(c[0]), floor(c[1])));
    // float2 b_r = random2(float2(floor(c[0])+1, floor(c[1])));
    // float2 t_l = random2(float2(floor(c[0]), floor(c[1])+1));
    // float2 t_r = random2(float2(floor(c[0])+1, floor(c[1])+1));
...
float perlin2d(float2 c)
{
    ...
    float2 new_c = float2(frac(c.x), frac(c.y));
    // float perlin = bicubicInterpolation(lst, new_c);
