    ...
    SubShader
    {
        Pass
        {
            CGPROGRAM
                ...
                {
                    v2f output;
                    // input.vertex.xyz = (input.vertex.xyz) + _BumpScale*(input.normal*perlin2d(input.uv * _NoiseScale));
                    float t = _Time.y * _TimeScale;
                    input.vertex.xyz = (input.vertex.xyz) + _BumpScale*(input.normal*waterNoise(input.uv * _NoiseScale, t));

                    
                    // output.norm = mul(unity_ObjectToWorld, normalize(input.vertex.xyz)); 
                    output.norm = mul(unity_ObjectToWorld, normalize(input.normal)); // TODO normalize(input.vertex)?
                ...
                fixed4 frag (v2f input) : SV_Target
                {
                    ...
                    //black&white
                    // float b = waterNoise(input.uv*_NoiseScale,0);
                    // return (b+1)/2;
