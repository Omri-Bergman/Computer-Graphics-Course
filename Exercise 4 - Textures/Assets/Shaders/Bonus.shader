Shader "CG/Bonus"
{
    Properties
    {
        ...
        [NoScaleOffset] _SpecularMap("Specular Map", 2D) = "" {}
        _Shininess("Shininess", Range(0.1, 100)) = 80
        [NoScaleOffset] _HeightMap("Height Map", 2D) = "" {}
        _BumpScale("Bump Scale", Range(1, 100)) = 3
        _BubbleColor("Bubble Color", Color) = (0.74, 0.83, 0.81, 1)
            ...
            CGPROGRAM
             ...
             float waterNoise(float2 uv, float t)
                ...
                {float t = _Time.y * _TimeScale;
                    ...
                    newBumpMapData.normal = normalize(input.normal);
                    newBumpMapData.tangent = normalize(input.tangent);
                    newBumpMapData.uv = input.uv;
                    newBumpMapData.heightMap = _HeightMap;
                    newBumpMapData.du = _HeightMap_TexelSize.x;
                    newBumpMapData.dv = _HeightMap_TexelSize.y;
                    newBumpMapData.bumpScale = _BumpScale / 10000;

                    float3 d =  _BumpScale*(input.normal*waterNoise(input.uv * _NoiseScale, t));
                    float3 n = normalize(getBumpMappedNormal(newBumpMapData))+d*10;
                    float3 l = normalize(_WorldSpaceLightPos0.xyz - (input.worldPos * _WorldSpaceLightPos0.w))+ d*10;
                    float3 v = normalize(_WorldSpaceCameraPos - input.worldPos.xyz)+ d*10;
                    ...
                    distort = lerp(distort, input.grabPos, _DistortionScale); 
                    fixed diameter = max(_FresnelIntensity * (_FresnelRadius - dot(n, v)), 0);  // Effect of fresnel.
                    float4 fresnel = float4(diameter, diameter, diameter, 0) * max(0.8, dot(n, l)) * _FresnelColor;  // Fresnel intensity 
                    fixed4 albedo = (tex2Dproj(_BackgroundTexture, distort) + tex2D(_AlbedoMap, input.uv))* _BubbleColor * _ColorScale ;
                    fixed4 specularity = tex2D(_SpecularMap, input.uv);
                    float3 new_n = newBumpMapData.normal;
